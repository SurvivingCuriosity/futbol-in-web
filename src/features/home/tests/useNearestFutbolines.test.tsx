...
// Factory limpia para SpotDTO
const makeSpot = (overrides: Partial<SpotDTO> = {}): SpotDTO => ({
  ...
  coordinates: [0, 0],
  direccion: "Calle X",
  distribucion: "F4" as any,
  googlePlaceId: "",
  idOperador: null,
  nombre: "Test",
  tipoFutbolin: "TSUNAMI" as any,
  tipoLugar: "FUBTOLIN" as any,
  verificado: null,
  votes: { up: [], down: [] },
  ...overrides,
});
...
it("ordena por distancia ascendente y toma la cantidad correcta", () => {
  ...
  const f1 = makeSpot({ coordinates: [0.001, 0] });  // el más cercano
  const f2 = makeSpot({ coordinates: [0.01, 0] });   // medio
  const f3 = makeSpot({ coordinates: [0.1, 0] });    // lejano
  ...
  it("devuelve distances truncadas", () => {
    ...
    const f1 = makeSpot({ coordinates: [-3.5, 40.4] });
