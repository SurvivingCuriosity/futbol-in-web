...
import { Incidencia, SpotDTO } from "futbol-in-core/types";
import { DetalleIncidencia } from "./DetalleIncidencia";

export const Incidencias = ({ futbolin }: { futbolin: SpotDTO }) => {
  const incidencias = futbolin.incidencias?.filter((i) => !i.resuelto) ?? [];

  const { openModal, closeModal } = useModal();

  if (incidencias.length === 0) return null;

  const handleOpenDetalleIncidencia = (i: Incidencia) => {
    openModal(
      <DetalleIncidencia
        futbolin={futbolin}
        incidencia={i}
