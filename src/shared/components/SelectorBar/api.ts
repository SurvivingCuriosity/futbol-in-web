...
export async function fetchBares(inputValue: string|number, ciudad:string = ""): Promise<PlaceOption[]> {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/maps/bares-autocomplete?input=${encodeURIComponent(ciudad+' '+inputValue)}`
  );
...
export const getCoordinatesFromPlaceId = async (placeId: string) => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/maps/getCoordinatesFromPlaceId?placeId=${placeId}`);
  const {data} = await res.json();
  return data;
}